...
object ArmeriaApplication {
    ...
    private val logger = LoggerFactory.getLogger(ArmeriaApplication::class.java)

    fun newServer(httpPort: Int, httpsPort: Int, useBlockingTaskExecutor: Boolean = false): Server {

        logger.info("httpsPort: {}", httpsPort)
        val exampleRequest: HelloRequest = HelloRequest.newBuilder().setName("HelloWorld").build()
        ...
        val grpcService = GrpcService.builder()
            ...
            // You can set useBlockingTaskExecutor(true) in order to execute all gRPC methods in the blockingTaskExecutor thread pool.
